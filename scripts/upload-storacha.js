 ...
 *
 * 获取 API 凭证步骤：
 * 1. 访问 https://storacha.network/
 * 2. 注册账户（Email 或 GitHub）
 * 3. 进入 Console → Settings → Create API Token
 * 4. 复制 Token 并添加到 .env: WEB3_STORAGE_TOKEN=your_token
 */
...
async function uploadToStoracha() {
  ...
  if (!TOKEN) {
    ...
    console.log("4. 点击 Settings → API Tokens");
    console.log("5. 点击 Create Token");
    console.log("6. 复制 Token");
    console.log("7. 添加到 .env: WEB3_STORAGE_TOKEN=your_token");
    console.log("\n✅ 免费 5GB 存储，我们只需要 ~9MB");
    process.exit(1);
  }
